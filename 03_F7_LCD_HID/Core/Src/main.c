...
static volatile uint16_t cursorX = 100, cursorY = 100;
...
void putcursor(void)
{
  if( cursorUpd )
  {
    ...
    BSP_LCD_SelectLayer(LCD_LAYER_FG);
    BSP_LCD_Clear(LCD_COLOR_WHITE);
    BSP_LCD_SetTextColor(0x88CC00FF);
    BSP_LCD_FillPolygon( points, pointNumber );
...
const int boardWidth = screenWidth / cellSize;
const int boardHeight = screenHeight / cellSize;
int boardGOL[17][30];
int tempBoardGOL[17][30];
...
int getAliveNeighbours(int i, int j) {
  return boardGOL[j][i + 1] + boardGOL[j - 1][i + 1] + boardGOL[j - 1][i] + boardGOL[j - 1][i - 1] + boardGOL[j][i - 1] + boardGOL[j + 1][i - 1] + boardGOL[j + 1][i] + boardGOL[j + 1][i + 1];
}
...
void stepGOL() {
    ...
    for(int i = 1; i < boardWidth - 1; i++) {
        for(int j = 1; j < boardHeight - 1; j++) {
            if(boardGOL[j][i] == 1 && (getAliveNeighbours(i, j) < 2 || getAliveNeighbours(i, j) > 3)) {
                tempBoardGOL[j][i] = 0;
            }
            if(boardGOL[j][i] == 1 && (getAliveNeighbours(i, j) == 2 || getAliveNeighbours(i, j) == 3)) {
                tempBoardGOL[j][i] = 1;
            }
            if(boardGOL[j][i] == 0 && (getAliveNeighbours(i, j) == 3)) {
                tempBoardGOL[j][i] = 1;
            }
        }
    }
    ...
    for(int i = 0; i < boardWidth; i++) {
        for(int j = 0; j < boardHeight; j++) {
            ...
            if(boardGOL[j][i] == 1) {
              ...
              BSP_LCD_FillRect( cellSize * j + 1, cellSize * i + 1, cellSize - 2, cellSize -2);
            }
